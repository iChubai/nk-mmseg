...
try:
    ...
    class Dict(dict):
        """Minimal addict.Dict-compatible fallback.

        Supports attribute access used by the local config loader.
        """
...
def load_cfg(filepath):
    ...
    if BASE_KEY in cfg:
        ...
        for base_cfg_path in base_cfg_paths:
            ...
            if not osp.isabs(base_cfg_path):
...
def merge_cfg(cfg_a, cfg_b):
    ...
    for k, v in cfg_b.items():
        if k in cfg_a and (isinstance(v, Mapping)
            ...
            if v.pop(COVER_KEY, False) or v.pop(DELETE_KEY, False):
                cfg_a[k] = v
            else:
                merge_cfg(cfg_a[k], v)
        else:
