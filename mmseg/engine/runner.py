...
class Runner:
    ...
    def __init__(self,
        ...
        elif self.cfg is not None and getattr(self.cfg, 'filename', None) is not None:
    ...
    def init_model_weights(self):
        ...
        def dfs_run_init_weights(model):
            ...
            init_fn = getattr(model, 'init_weights', None)
            if init_fn is None or not callable(init_fn):
                return
            if getattr(model, '_is_init', False):
                return
            if not can_call_no_args(init_fn):
                return
            init_fn()
    ...
    def setup_logger_dir(self, phase):
        ...
        self.logger = get_logger(name='mmseg-jittor', log_file=log_file)
