...
class HRNet(BaseModule):
    ...
    def forward(self, x):
        ...
        for i in range(self.stage3_cfg['num_branches']):
            # In this Jittor port, None placeholders in ModuleList are
            # represented by Identity() for container compatibility.
            if self.transition2[i] is not None and \
                    not isinstance(self.transition2[i], nn.Identity):
        ...
        for i in range(self.stage4_cfg['num_branches']):
            if self.transition3[i] is not None and \
                    not isinstance(self.transition3[i], nn.Identity):
