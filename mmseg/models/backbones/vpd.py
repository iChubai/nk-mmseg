...
class VPD(BaseModule):
    ...
    def __init__(self,
                 diffusion_cfg: Optional[ConfigType] = None,
                 class_embed_path: Optional[str] = None,
                 unet_cfg: OptConfigType = None,
                 gamma: float = 1e-4,
        ...
        self._ldm_available = has_ldm

        if not has_ldm:
            warnings.warn(
                'VPD optional dependencies are not installed. '
                'Falling back to a lightweight convolutional backbone for '
                'pure-Jittor runtime compatibility. Install '
                'requirements/optional.txt to enable full VPD diffusion '
                'backbone.',
                stacklevel=2)
            text_dim = 768
            self.encoder_vq = nn.Identity()
            self.unet = nn.Identity()
            self.fallback_stage0 = nn.Sequential(
                nn.Conv2d(3, 320, kernel_size=3, stride=2, padding=1),
                nn.GroupNorm(16, 320),
                nn.SiLU(),
            )
            self.fallback_stage1 = nn.Sequential(
                nn.Conv2d(320, 640, kernel_size=3, stride=2, padding=1),
                nn.GroupNorm(32, 640),
                nn.SiLU(),
            )
            self.fallback_stage2 = nn.Sequential(
                nn.Conv2d(640, 1280, kernel_size=3, stride=2, padding=1),
                nn.GroupNorm(32, 1280),
                nn.SiLU(),
            )
            self.fallback_stage3 = nn.Sequential(
                nn.Conv2d(1280, 1280, kernel_size=3, stride=2, padding=1),
                nn.GroupNorm(32, 1280),
                nn.SiLU(),
            )
            self.text_adapter = TextAdapter(text_dim=text_dim)
            self.register_buffer('class_embeddings', torch.zeros((1, text_dim)))
            self.gamma = nn.Parameter(torch.ones(text_dim) * gamma)
            return

        if diffusion_cfg is None:
            raise ValueError('VPD requires `diffusion_cfg`.')
        unet_cfg = dict(unet_cfg or {})
        ...
        if class_embed_path is None:
            class_embeddings = torch.zeros((1, 768))
        else:
            class_embeddings = CheckpointLoader.load_checkpoint(class_embed_path)
            if isinstance(class_embeddings, dict):
                if 'state_dict' in class_embeddings:
                    state_dict = class_embeddings['state_dict']
                else:
                    state_dict = class_embeddings
                tensor_val = next(
                    (v for v in state_dict.values() if isinstance(v, torch.Tensor)),
                    None)
                if tensor_val is None:
                    raise TypeError(
                        f'Unable to find tensor embeddings in checkpoint: '
                        f'{class_embed_path}')
                class_embeddings = tensor_val
        if class_embeddings.ndim == 1:
    ...
    def forward(self, x):
        ...
        if not self._ldm_available:
            ...
            # VPDDepthHead expects stage-0 features roughly at 1/8 image scale.
            # Emulate diffusion latents by reducing image resolution before the
            # fallback feature pyramid.
            latent_h = max(1, int(x.shape[-2]) // 4)
            latent_w = max(1, int(x.shape[-1]) // 4)
            x = F.interpolate(
                x,
                size=(latent_h, latent_w),
                mode='bilinear',
                align_corners=False)
            out0 = self.fallback_stage0(x)
            out1 = self.fallback_stage1(out0)
            out2 = self.fallback_stage2(out1)
            out3 = self.fallback_stage3(out2)
