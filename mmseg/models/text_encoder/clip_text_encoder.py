...
class CLIPTextEncoder(BaseModule):
    ...
    def __init__(self,
        ...
        self.final_name, ln_final = build_norm_layer(
            norm_cfg, embed_dims, postfix='_final_layer')
        self.add_module(self.final_name, ln_final)
    ...
    def forward(self):
        """Forward function."""
        if self.dataset_name is None and self.vocabulary is None:
            raise RuntimeError(
                "text_encoder forward required either 'dataset_name' or "
                "'vocabulary'")
