...
class DAPPM(BaseModule):
    ...
    def __init__(self,
        ...
        self.pool_kernel_sizes = list(kernel_sizes)
        self.pool_strides = list(strides)
        self.pool_paddings = list(paddings)
    ...
    def _safe_avg_pool_branch(self, branch_index: int, inputs: Tensor) -> Tensor:
        if branch_index == self.num_scales - 1:
            return self.scales[branch_index](inputs)
        k = int(self.pool_kernel_sizes[branch_index - 1])
        s = int(self.pool_strides[branch_index - 1])
        p = int(self.pool_paddings[branch_index - 1])
        h, w = int(inputs.shape[2]), int(inputs.shape[3])
        out_h = max(1, (h + 2 * p - k) // s + 1)
        out_w = max(1, (w + 2 * p - k) // s + 1)
        pooled = F.adaptive_avg_pool2d(inputs, output_size=(out_h, out_w))
        # self.scales[branch_index] is [AvgPool2d, ConvModule] in DAPPM.
        return self.scales[branch_index][1](pooled)
    ...
    def forward(self, inputs: Tensor):
        ...
        for i in range(1, self.num_scales):
            feat_up = F.interpolate(
                self._safe_avg_pool_branch(i, inputs),
                size=inputs.shape[2:],
