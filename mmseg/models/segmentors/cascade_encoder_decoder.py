...
class CascadeEncoderDecoder(EncoderDecoder):
    ...
    def _forward(self,
        ...
        if self.num_stages == 1:
            head = self.decode_head[0]
            head_forward = getattr(head, 'forward', None)
            if callable(head_forward):
                try:
                    return head_forward(x)
                except (AttributeError, NotImplementedError, TypeError):
                    pass
            return head.predict(x, batch_img_metas, self.test_cfg)

        out = self.decode_head[0].forward(x)
        for i in range(1, self.num_stages - 1):
            out = self.decode_head[i].forward(x, out)

        last_head = self.decode_head[-1]
        last_forward = getattr(last_head, 'forward', None)
        if callable(last_forward):
            try:
                return last_forward(x, out)
            except (AttributeError, NotImplementedError, TypeError):
                pass

        return last_head.predict(x, out, batch_img_metas, self.test_cfg)
